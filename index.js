...
var vk = new VK({
	token:
		"vk1.a.dr1zSSLecNowQpyaM-Uf7FVY63vj4bff2xaLQZc8DhGBuazEW7QKOJ2QYhCBqYtQc-WX2mOhHxeI91NlB982HtlGXoN1Y8uWhzDhP_iDUHDTmnp7TpBRXrzeAX-NdjTVxnxCxy5mjFYwSlcvZhUlczRLzYw-rxoy7HfH0icHV575Gru1y0MVQPg2U1ryOuZ4Nix3zVnkhMqhziqT5rIL1A",
});
const topsum_ads = [
	250_000_000,
	175_000_000,
...
const bot = new Telegraf("6832708607:AAFESrH5rM3QKHzsNn-sf42vd93xFNQgV-4");
var {
	getUnix,
	getUrlVars,
	validateAppUrl,
...
var admins = [578382301, 239489178];
var owners = [578382301];
const bea_format = (v) => {
	if (v >= 1000 && v < 1000000) {
...
vk.updates.on("message_new", async (message) => {
	...
	if (message.text.match(/^(\/|)(начать|app|start)$/i)) {
		return message.send(`Приложение: https://vk.com/app51731291\nКошелек: https://vk.com/app51539154`);
	}
	if (message.text.match(/^\/post.(.+)$/i)) {
		...
		if (message.peerId !== 2000000040) return;
		...
		if (user) {
			...
			const link = await vk.api.utils.getShortLink({
				url: `https://t.me/SpadesCoinBot?start=${user.code}`,
			});
			return message.send(
				`Перейдите по ссылке, чтобы привязать ТГ. \n\nСсылка: ${
					link.short_url ?? `https://t.me/SpadesCoinBot?start=${user.code}`
				}`
			);
		}
		...
		var link = await vk.api.utils.getShortLink({
			url: `https://vk.com/app51731291#r_${message.senderId}`,
		});
		return message.send(
			`Ваша ссылка: ${
				link.short_url
					? link.short_url
					: "https://vk.com/app51731291#r_" + message.senderId
